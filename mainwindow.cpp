...
//void QThread *thread = QThread::create([]{
//    while(1)
...
while (1 > 0) {
 QEventLoop loop;
    ...
    QRController* controller = new QRController(ui->lineEdit->text().toUtf8().constData(), eclHigh, qrcode_width, qrcode_height, evAuto);
    int suitablepixlength = 0;
    bool checkresult = controller->getSuitablePixLength(suitablepixlength);
    if (true == checkresult){
        delete controller;
#ifdef DEBUG_LOGOUT_ON
                  controller = new QRController(ui->lineEdit->text().toUtf8().constData(), eclHigh, suitablepixlength, suitablepixlength, evAuto);
        qDebug("loaded random QR code");
#endif
...
#ifdef DEBUG_LOGOUT_ON
//                  controller = new QRController(ui->lineEdit->text().toUtf8().constData(), eclHigh, suitablepixlength, suitablepixlength, evAuto);
        qDebug("loaded random QR code loop");
#endif
//      on_generateButton_pressed_r();
...
void MainWindow::on_generateButton_pressed_r()
{
// QThread on_generateButton_pressed; 
//    on_generateButton_pressed.start(); 
